...
function startGame(){
  ...
  if(_scenes && _scenes.length){   // 章=シーン連結(複数幕の連続)
    ...
    showChapterTitle(stage,()=>{ chap.titleShown=true; playScene(); }); return;
  }
  chap=null;
...
function interlude(next){
  ...
  box.innerHTML=`<div class="il-mark">― 第${chap.idx+1}幕 ／ ${chap.scenes.length} ―</div>
    ...
    <pre class="il-text"></pre>`;
  const ov=$('#sceneov'), pre=box.querySelector('pre'), text=sc.pre||'物語は続く。';
  let i=0, done=false, advanced=false, timer=null;
  function addButton(){ if(box.querySelector('#il-go'))return; const b=el('button','btn primary','進む'); b.id='il-go'; b.onclick=e=>{ e.stopPropagation(); advance(); }; box.appendChild(b); }
  function finish(){ if(done)return; done=true; clearInterval(timer); pre.textContent=text; addButton(); }
  function advance(){ if(advanced)return; if(!done){ finish(); return; } advanced=true; clearInterval(timer); ov.onclick=null; ov.classList.remove('show'); next(); }
  timer=setInterval(()=>{ i++; pre.textContent=text.slice(0,i); if(i>=text.length)finish(); },15);
  ov.onclick=advance;
  ov.classList.add('show');
}
