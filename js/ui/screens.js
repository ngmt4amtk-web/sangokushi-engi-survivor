...
function experienceText(){
  const n=Object.keys(window.Save.get().cleared||{}).length;
  return `演義体験率 ${n}/120回 (${Math.round(n/120*100)}%)`;
}
function rankCls(r){ return r==='S'?'s':(r==='A'?'a':(r==='B'?'b':(r==='天命'?'destiny':''))); }
function chapterDoomOnly(stage){
  const sc=chapterScenesFor(stage);
  return !!(sc&&sc.length&&sc.every(x=>x.kind==='doom'));
}
function calcRank(res){
  if(!res||!res.win)return '';
  if(chapterDoomOnly(res.stage))return '天命';
  const hits=res.hits||0;
  return hits===0?'S':(hits<10?'A':'B');
}
function applyAudioPrefs(){
  const o=(window.Save.get().opts||{});
  window.SFX&&SFX.setMuted(!!o.muted);
  window.SFX&&SFX.setVolume(Math.max(0,Math.min(1,((o.sfxVolume==null?80:o.sfxVolume)/100)*0.45)));
  window.BGM&&BGM.setVolume(o.bgmVolume==null?55:o.bgmVolume);
  window.BGM&&BGM.setMuted(!!o.muted);
}
function screenBgm(id){
  if(!window.BGM||!id)return;
  if(['s-title','s-stage','s-lord','s-codex','s-read','s-barracks','s-achievements'].includes(id)) BGM.play('title');
}
...
function show(id){
  ...
  if(id){ const ub=$('#ultbtn'); if(ub)ub.style.display='none'; const lv=$('#lowhpvignette'); if(lv)lv.className=''; const s=$('#'+id); s.classList.add('show'); s.scrollTop=0; screenBgm(id); }   // 遷移先は必ず先頭から(前画面のスクロール位置を持ち越さない)
}
...
function renderTitle(){
  ...
  c.innerHTML=`
    ...
    <div class="tl-menu">
      ...
      <button class="btn tl-menu-btn" id="b-achievements">実績帳</button>
      <button class="btn tl-menu-btn" id="b-codex">英雄図鑑</button>
      <button class="btn tl-menu-btn" id="b-barracks">修練場</button>
      <button class="btn tl-menu-btn" id="b-settings">設定</button>
    </div>
  ...
  $('#b-achievements').onclick=()=>{renderAchievements();show('s-achievements');};
  ...
  $('#b-settings').onclick=()=>renderSettings();
...
function renderStage(){
  ...
  let h=`<div class="topbar"><h2 class="sec" style="margin:0;border:none;">出陣 ─ 戦いを選べ（全120）</h2><div class="exp-rate">${experienceText()}</div></div>
  ...
  for(const st of window.STAGES){
    ...
    h+=`<div class="scard ${ok?'':'locked'} ${cleared?'cleared':''}" data-st="${st.id}" ${ok?'':'data-lock=1'}>
      ...
      <div class="srow">
        ...
        ${rk?`<span class="rank-badge ${rankCls(rk)}">${rk}</span>`:''}
...
function renderPrep(){
  ...
  // 主役は物語が決める(選択廃止)。表示は第1幕の主役のみ=後続幕・幕数はネタバレ防止で隠す
  const firstId=(_scn&&_scn.length&&_scn[0].proto!=null)?_scn[0].proto:(st.protagonist||[])[0];
  let protoCands=[genById(firstId)].filter(Boolean);
  selProto=protoCands[0]||null;
  ...
  // 主役ドット絵セクション
  h+=`<div class="prep-lead">今回の主役</div>`;
...
function showScenePre(sc, next){
  ...
  const markHtml = sc.markText
    ...
    : (chap
    ? `<div class="tw-mark">${esc(sc.name||'')}</div>`
    : '');
...
function startGame(){
  ...
  if(_scenes && _scenes.length){   // 章=シーン連結(複数幕の連続)
    chap={stage, scenes:_scenes, idx:0, titleShown:false, accum:{kills:0,gold:0,level:1,time:0,hits:0,ultNames:[],namedKillSec:{},bossKillSec:null,bossName:null}};
    // 章タイトルカード → 第1幕pre → 戦闘
...
function playScene(){
  ...
  window.BGM&&BGM.play(sc.kind==='doom'?'doom':'battle');
  show(null); $('#hud').classList.add('show'); $('#pausebtn').style.display='block'; lastWsig='';
  G.startRun({lord, stage:sceneStageOf(chap.stage,sc), owned:S.owned, save:S, difficulty:selDiff, curse:selCurse,
    scene:{kind:sc.kind, dur:sc.dur, name:sc.name||'', last, deathLine:sc.deathLine, epitaph:sc.epitaph, bossAt:sc.bossAt}});
  sceneIntro(sc,gen);
}
function sceneIntro(sc,gen){
  ...
  t.innerHTML=`<div style="font-size:12px;color:var(--txt2)">${kl}</div>
...
function onSceneEnd(reason,res){
  ...
  chap.accum.hits+=(res.hits||0);
  chap.accum.ultNames=chap.accum.ultNames.concat(res.ultNames||[]);
  Object.assign(chap.accum.namedKillSec,res.namedKillSec||{});
  if(res.bossKillSec!=null){ chap.accum.bossKillSec=res.bossKillSec; chap.accum.bossName=res.bossName; }
...
function finishChapter(win,res){
  ...
  const merged=Object.assign({},res,{win, stage:chap.stage, kills:chap.accum.kills, gold:chap.accum.gold, time:chap.accum.time, level:chap.accum.level, hits:chap.accum.hits,
    ultNames:[...new Set(chap.accum.ultNames)], namedKillSec:Object.assign({},chap.accum.namedKillSec),
    bossKillSec:chap.accum.bossKillSec, bossName:chap.accum.bossName, lord:{name:names.join('・')}});
  chap=null;
  onEnd(merged);
}
...
function onEnd(res){
  ...
  window.Save.save();
  if(res.win) window.BGM&&BGM.play('victory');
  else window.BGM&&BGM.stop();
  window.ACH&&ACH.onRunEnd(res);
  lastResult=res;
  const box=$('#resultbox');
  const rk=res.rank?`<div class="res-rank ${rankCls(res.rank)}">${res.rank}</div>`:'';
  box.innerHTML=`
    ...
    <div class="res-stat">
      ...
      <div class="k">被弾</div><div class="v">${res.hits||0}</div>
    </div>
  ...
  if(res.win){
    const share=el('button','btn share-btn','戦果を共有');
    share.onclick=()=>shareResult(res);
...
function shareResult(res){
  ...
  const url='https://ngmt4amtk-web.github.io/sangokushi-engi-survivor/';
  const sub=res.stage.sub?(' - '+res.stage.sub):'';
  const text=`三国志演義survivor 第${res.stage.no}回『${res.stage.name}』踏破${sub}。演義体験率${pct}% ${url}`;
  if(navigator.share){
...
function renderPause(){
  ...
  const setMt=()=>mt.textContent='全音ミュート '+((window.Save.get().opts||{}).muted?'ON':'OFF');
  setMt(); mt.onclick=()=>{ const S=window.Save.get(); S.opts=S.opts||{}; S.opts.muted=!S.opts.muted; window.Save.save(); applyAudioPrefs(); setMt(); };
  box.appendChild(mt);
  ...
  box.appendChild(au);
  const st=el('button','lvbtn','設定を開く'); st.style.marginTop='8px'; st.onclick=()=>renderSettings();
  box.appendChild(st);
}
...
function renderAchievements(){
  const c=$('#s-achievements .content');
  const list=(window.ACH&&ACH.all())||[];
  const got=list.filter(a=>a.unlocked).length;
  c.innerHTML=`<div class="topbar"><h2 class="sec" style="margin:0;border:none;">実績帳</h2><div class="ach-count">${got}/${list.length}</div></div>
    <div class="note">未解除はシルエットとヒントだけ表示。解除後に演義トリビアが開く。</div>
    <div class="ach-grid">${list.map(a=>`<div class="ach-card ${a.unlocked?'on':'off'}">
      <div class="ach-seal">${a.unlocked?'演':'?'}</div>
      <div class="ach-name">${esc(a.name)}</div>
      <div class="ach-hint">${esc(a.unlocked?a.trivia:a.hint)}</div>
    </div>`).join('')}</div>`;
}

// ── 設定/セーブ入出力 ───────────────────────
function renderSettings(){
  let ov=$('#settings'), box=$('#settingsbox');
  if(!ov){
    ov=el('div','modal'); ov.id='settings';
    box=el('div','sheet settings-sheet'); box.id='settingsbox';
    ov.appendChild(box); document.body.appendChild(ov);
  }
  const S=window.Save.get(); S.opts=S.opts||{}; const o=S.opts;
  const val=(k,d)=>o[k]===undefined?d:o[k];
  box.innerHTML=`
    <h3>設定</h3>
    <div class="set-row"><span>BGM音量</span><input id="set-bgm" type="range" min="0" max="100" value="${val('bgmVolume',55)}"><b id="set-bgm-v">${val('bgmVolume',55)}</b></div>
    <div class="set-row"><span>SFX音量</span><input id="set-sfx" type="range" min="0" max="100" value="${val('sfxVolume',80)}"><b id="set-sfx-v">${val('sfxVolume',80)}</b></div>
    <label class="set-check"><input id="set-muted" type="checkbox" ${val('muted',false)?'checked':''}> 全音ミュート</label>
    <label class="set-check"><input id="set-shake" type="checkbox" ${val('shake',true)?'checked':''}> 画面シェイク</label>
    ...
    <div class="save-tools">
      ...
      <div class="row">
        <button class="btn" id="save-export">JSON書出</button>
        <button class="btn" id="save-import">JSON読込</button>
      </div>
    </div>
    <button class="btn primary" id="settings-close">閉じる</button>`;
  function put(k,v){ S.opts[k]=v; window.Save.save(); applyAudioPrefs(); if(k==='autoUlt')G.setAutoUlt&&G.setAutoUlt(v); }
  const bgm=$('#set-bgm'), sfx=$('#set-sfx');
  bgm.oninput=e=>{ const v=+e.target.value; $('#set-bgm-v').textContent=v; put('bgmVolume',v); };
  sfx.oninput=e=>{ const v=+e.target.value; $('#set-sfx-v').textContent=v; put('sfxVolume',v); };
  $('#set-muted').onchange=e=>put('muted',e.target.checked);
  $('#set-shake').onchange=e=>put('shake',e.target.checked);
  ...
  $('#save-export').onclick=async()=>{
    ...
    $('#save-json').value=txt;
    try{ if(navigator.clipboard) await navigator.clipboard.writeText(txt); uiToast('セーブJSONを書き出した'); }
    catch(e){ uiToast('JSONを表示した'); }
  };
  $('#save-import').onclick=()=>{
    try{
      window.Save.importJson($('#save-json').value);
      applyAudioPrefs();
      window.ACH&&ACH.checkCollection();
      uiToast('セーブJSONを読み込んだ');
      ov.classList.remove('show');
      renderTitle();
    }catch(e){ uiToast(e.message||'読込に失敗した'); }
  };
  $('#settings-close').onclick=()=>ov.classList.remove('show');
  ov.classList.add('show');
}
...
function init(){
  ...
  G.onToast=uiToast;
  G.onKizunaFuse=(kz)=>{ window.ACH&&ACH.event('kizunaFuse',{id:kz.id}); };
  applyAudioPrefs();
