...
function showScenePre(sc, next){
  ...
  box.innerHTML=`${markHtml}<pre class="tw-text"></pre>`;
  const pre=box.querySelector('.tw-text');
  let i=0, done=false, advanced=false, tid=null;
  function finish(){ if(done)return; done=true; clearInterval(tid); pre.textContent=text; }
  function advance(){
    if(advanced)return;
    if(!done){ finish(); return; }
    advanced=true; clearInterval(tid); ov.onclick=null;
    ov.classList.remove('show'); ov.classList.remove('tw-bg');
    next();
  }
  tid=setInterval(()=>{ i++; pre.textContent=text.slice(0,i); if(i>=text.length)finish(); },8);
  ov.onclick=advance;
  ov.classList.add('show');
